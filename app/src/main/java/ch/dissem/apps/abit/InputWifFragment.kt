    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        if (view != null) {
            view.findViewById<Button>(R.id.next).setOnClickListener {
                val bundle = Bundle()
                bundle.putString(ImportIdentitiesFragment.WIF_DATA, view.findViewById<TextView>(R.id.wif_input).text.toString())
            ...
            if (files.isNotEmpty()) {
                try {
                    FileInputStream(files[0]).use { inputStream ->
                        ...
                        view.findViewById<TextView>(R.id.wif_input).setText(data.toByteArray().toString())
                    }
                } catch (e: IOException) {
                    Toast.makeText(
                            activity,
