...
class SettingsFragment : PreferenceFragmentCompat(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        findPreference<Preference>("about")?.onPreferenceClickListener = aboutClickListener()
        val cleanup = findPreference<Preference>("cleanup")
        cleanup?.onPreferenceClickListener = cleanup?.let { cleanupClickListener(it) }
        findPreference<Preference>("export")?.onPreferenceClickListener = exportClickListener()
        findPreference<Preference>("import")?.onPreferenceClickListener = importClickListener()
        findPreference<Preference>("status")?.onPreferenceClickListener = statusClickListener()
    }
    ...
    override fun onAttach(activity: Activity) {
        ...
        (activity as? MainActivity)?.floatingActionButton?.hide()
        PreferenceManager.getDefaultSharedPreferences(activity)
            .registerOnSharedPreferenceChangeListener(this)

        (activity as? MainActivity)?.updateTitle(getString(R.string.settings))
    }
