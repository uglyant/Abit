...
class AddressDetailFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        item?.let { item ->
            ...
            view.findViewById<ImageView>(R.id.avatar).setImageDrawable(Identicon(item))
            view.findViewById<TextView>(R.id.name).setText(item.toString())
            view.findViewById<TextView>(R.id.name).addTextChangedListener(object : TextWatcher {
            ...
            view.findViewById<TextView>(R.id.address).isSelected = true
            view.findViewById<TextView>(R.id.stream_number).text = getString(R.string.stream_number, item.stream)
            if (item.privateKey == null) {
                view.findViewById<Switch>(R.id.active).isChecked = item.isSubscribed
                view.findViewById<Switch>(R.id.active).setOnCheckedChangeListener { _, checked -> item.isSubscribed = checked }

                if (item.pubkey == null) {
                    view.findViewById<ImageView>(R.id.pubkey_available).alpha = 0.3f
                    view.findViewById<TextView>(R.id.pubkey_available_desc).setText(R.string.pubkey_not_available)
                }
            } else {
                view.findViewById<Switch>(R.id.active).visibility = View.GONE
                view.findViewById<ImageView>(R.id.pubkey_available).visibility = View.GONE
                view.findViewById<TextView>(R.id.pubkey_available_desc).visibility = View.GONE
            }

            // QR code
            view.findViewById<ImageView>(R.id.qr_code).setImageBitmap(Drawables.qrCode(item))
        }
    }

    override fun onPause() {
        item?.let { item ->
            Singleton.getAddressRepository(requireContext()).save(item)
            if (item.privateKey != null) {
