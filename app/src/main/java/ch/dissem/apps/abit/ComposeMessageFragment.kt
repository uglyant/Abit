...
class ComposeMessageFragment : Fragment() {
    ...
    private var recipient_input: AutoCompleteTextView? = null
    private var sender_input: Spinner? = null
    private var subject_input: EditText? = null
    private var body_input: EditText? = null
        ...
        arguments?.apply {
            ...
            if (draft != null) {
                ...
                if (context != null && (id == null || id.privateKey == null)) {
                    id = Singleton.getIdentity(requireContext())
                }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        ///view.findViewById<Spinner>(R.id.sender_input)
        recipient_input =  view.findViewById<AutoCompleteTextView>(R.id.recipient_input)
        sender_input =  view.findViewById<Spinner>(R.id.sender_input)
        subject_input =  view.findViewById<EditText>(R.id.subject_input)
        body_input =  view.findViewById<EditText>(R.id.body_input)
        context?.let { ctx ->
            val identities = Singleton.getAddressRepository(ctx).getIdentities()
            view.findViewById<Spinner>(R.id.sender_input).adapter = ContactAdapter(ctx, identities, true)
            val index = identities.indexOf(Singleton.getIdentity(ctx))
            if (index >= 0) {
                view.findViewById<Spinner>(R.id.sender_input).setSelection(index)
            }

            if (broadcast) {
                view.findViewById<AutoCompleteTextView>(R.id.recipient_input).visibility = View.GONE
            } else {
                val adapter = ContactAdapter(
                    ctx,
                    Singleton.getAddressRepository(ctx).getContacts()
                )
                view.findViewById<AutoCompleteTextView>(R.id.recipient_input).setAdapter(adapter)
                view.findViewById<AutoCompleteTextView>(R.id.recipient_input).onItemClickListener =
                    AdapterView.OnItemClickListener { _, _, pos, _ -> recipient = adapter.getItem(pos) }

                view.findViewById<AutoCompleteTextView>(R.id.recipient_input).onItemSelectedListener =
                ...
                recipient?.let { view.findViewById<AutoCompleteTextView>(R.id.recipient_input).setText(it.toString()) }
            }
            view.findViewById<EditText>(R.id.subject_input).setText(subject)
            view.findViewById<EditText>(R.id.body_input).setText(content)

            when {
                recipient == null -> view.findViewById<AutoCompleteTextView>(R.id.recipient_input).requestFocus()
                subject.isEmpty() -> view.findViewById<AutoCompleteTextView>(R.id.recipient_input).requestFocus()
                else -> {
                    view.findViewById<EditText>(R.id.body_input).requestFocus()
                    view.findViewById<EditText>(R.id.body_input).setSelection(0)
                }
            }
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.select_encoding -> {
                ...
                fragmentManager?.let { encodingDialog.show(it, "select encoding dialog") }
                return true
            }
    ...
    private fun build(ctx: Context): Plaintext {
        ...
        when (encoding) {
            ...
            Plaintext.Encoding.EXTENDED -> builder.message(
              ...
              ///  getString(R.string.error_unsupported_encoding, encoding)
                Toast.makeText(
                    ctx,
                    ctx.getString(R.string.error_unsupported_encoding),
                    Toast.LENGTH_LONG
                ).show()
                builder.message(
                    subject_input?.text.toString(),
