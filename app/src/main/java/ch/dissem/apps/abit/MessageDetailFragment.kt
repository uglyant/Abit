...
class MessageDetailFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        item?.let { item ->
            ...
            view.findViewById<ImageView>(R.id.avatar).setOnClickListener(senderClickListener)
            view.findViewById<TextView>(R.id.sender).setOnClickListener(senderClickListener)
            view.findViewById<TextView>(R.id.sender).text = item.from.toString()
            item.to?.let { to ->
                view.findViewById<TextView>(R.id.recipient).text = to.toString()
            } ?: {
                if (item.type == Plaintext.Type.BROADCAST) {
                    view.findViewById<TextView>(R.id.recipient).setText(R.string.broadcast)
                }
            }.invoke()
            val labelAdapter = LabelAdapter(ctx, item.labels)
            view.findViewById<RecyclerView>(R.id.labels).adapter = labelAdapter
            view.findViewById<RecyclerView>(R.id.labels).layoutManager = GridLayoutManager(activity, 2)

            view.findViewById<TextView>(R.id.text).text = item.text

            Linkify.addLinks(view.findViewById<TextView>(R.id.text), WEB_URLS)
            Linkify.addLinks(view.findViewById<TextView>(R.id.text), BITMESSAGE_ADDRESS_PATTERN, BITMESSAGE_URL_SCHEMA, null,
                Linkify.TransformFilter { match, _ -> match.group() }
            )

            view.findViewById<TextView>(R.id.text).linksClickable = true
            view.findViewById<TextView>(R.id.text).setTextIsSelectable(true)
