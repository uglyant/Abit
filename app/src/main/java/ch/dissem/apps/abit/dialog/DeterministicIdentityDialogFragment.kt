...
class DeterministicIdentityDialogFragment : AppCompatDialogFragment() {
    ...
    override fun onViewCreated(dialogView: View, savedInstanceState: Bundle?) {
        ...
        dialogView.findViewById<Button>(R.id.ok).setOnClickListener {
            ...
            val passphraseText = dialogView.findViewById<TextView>(R.id.passphrase).text.toString()
            ...
            doAsync {
                val identities = bmc.createDeterministicAddresses(
                        passphraseText,
                    dialogView.findViewById<EditText>(R.id.number_of_identities).text.toString().toInt(),
                        Pubkey.LATEST_VERSION,
                        1L,
                    dialogView.findViewById<Switch>(R.id.shorter).isChecked
                )
                for ((i, identity) in identities.withIndex()) {
                    if (identities.size == 1) {
                        identity.alias = dialogView.findViewById<EditText>(R.id.label).text.toString()
                    } else {
                        identity.alias = "${dialogView.findViewById<EditText>(R.id.label).text} (${i + 1})"
                    }
        ...
        dialogView.findViewById<Button>(R.id.dismiss).setOnClickListener { dismiss() }
    }
